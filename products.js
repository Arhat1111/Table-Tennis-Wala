...
async function loadProductsData(){
  ...
  if(location.protocol==='http:'||location.protocol==='https:'){
    try{
      ...
      const response=await fetch(storeApiUrl('/api/site-data'),{cache:'no-store',signal:controller.signal});
      clearTimeout(timer);
      if(response.ok){const remote=await response.json();if(remote?.products&&remote?.commerce)siteData=remote;}
    }catch(e){console.info('InkWave: using bundled catalog data.');}
  }
