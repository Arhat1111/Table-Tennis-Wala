...
function browserAdminData(){try{const raw=localStorage.getItem('inkwaves-admin-local-v54-empty');if(!raw)return null;const parsed=JSON.parse(raw);return parsed?.products&&parsed?.commerce?parsed:null}catch{return null}}
...
async function loadData(){
  siteData=(location.protocol==='file:'?browserAdminData():null)||(window.INKWAVE_DEFAULT_DATA?clone(window.INKWAVE_DEFAULT_DATA):null);
  if(location.protocol==='http:'||location.protocol==='https:'){
    try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),2500);const r=await fetch(storeApiUrl('/api/site-data'),{cache:'no-store',signal:controller.signal});clearTimeout(timer);if(r.ok){const remote=await r.json();if(remote?.products&&remote?.commerce)siteData=remote}}catch{console.info('InkWaves: using bundled product data.');}
  }
...
function mockupMarkup(image,name,type){
  const zoom=type==='detail'?' clean-product-image-detail':'';
  const label=type==='detail'?'02 · DETAIL VIEW':'01 · PRODUCT VIEW';
  return `<article class="product-mockup-slide clean-product-slide${zoom}"><span class="mockup-view-label">${label}</span><div class="clean-product-image-wrap"><img class="mockup-poster-art" referrerpolicy="no-referrer" src="${esc(image)}" alt="${esc(name)}" /></div></article>`;
}
function renderMockups(){
  const track=$('#productMockupTrack');
  track.innerHTML=mockupMarkup(product.image,product.name,'main')+mockupMarkup(product.image,product.name,'detail');
  $('#productMockupDots').innerHTML='<button class="active" data-mockup-dot="0" aria-label="Product view"></button><button data-mockup-dot="1" aria-label="Detail view"></button>';
  mockupIndex=0;updateMockupSlider();wireImageFallbacks(track)
}
function updateMockupSlider(){
  const track=$('#productMockupTrack');
  track.style.transform=`translateX(-${mockupIndex*100}%)`;
  document.querySelectorAll('[data-mockup-dot]').forEach((d,i)=>d.classList.toggle('active',i===mockupIndex));
  $('#productMockupCounter').textContent=`0${mockupIndex+1} / 02`
}
function changeMockup(delta){mockupIndex=(mockupIndex+delta+2)%2;updateMockupSlider()}
function render(){
